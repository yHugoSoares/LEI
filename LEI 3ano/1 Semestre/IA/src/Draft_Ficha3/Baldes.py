...
class Balde():
    ...
    def cria_grafo(self):
        ...
        while estados != []:
            estado_atual = estados.pop()

            expansao = self.expande(estado_atual)
            for e in expansao:
                self.g.addEdge(estado_atual, e, weight=1)
    ...
    def expande(self,estado):
        lista_estados = []
        cap1 = int(lista_estados[0])
        cap2 = int(lista_estados[3])
        if cap1 > 0:
            lista_estados.append(self.esvazia1(estado))
        if cap2 > 0:
            lista_estados.append(self.esvazia2(estado))

        if cap1 < self.balde1:
            lista_estados.append(self.enche1(estado))
        if cap2 < self.balde2:
            lista_estados.append(self.enche2(estado))
        
        if cap1 > 0 and cap2 < self.balde2:
            lista_estados.append(self.despeja12(estado))
        if cap2 > 0 and cap1 < self.balde1:
            lista_estados.append(self.despeja21(estado))
