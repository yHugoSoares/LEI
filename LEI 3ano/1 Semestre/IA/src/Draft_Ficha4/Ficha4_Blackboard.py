...
def minimax(estado, profundidade, jogador):
    ...
    # Escolha inicial dependendo se estamos a maximizar (BOT) ou minimizar (HUMANO)
    if jogador == BOT:
        melhor = [-1, -1, -infinito]
    else:
        melhor = [-1, -1, infinito]

    for (x, y) in celulas_vazias(estado):
        # Testa jogada
        estado[x][y] = jogador
        resultado = minimax(estado, profundidade - 1, -jogador)
        # Reverte jogada
        estado[x][y] = 0
