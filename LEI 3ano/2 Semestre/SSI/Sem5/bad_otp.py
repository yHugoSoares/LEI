...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 bad_otp.py <setup|enc|dec> ...")
        sys.exit(1)
    ...
    if mode == "setup":
        if len(sys.argv) != 4:
            print("Usage: python3 bad_otp.py setup <num_bytes> <key_file>")
            sys.exit(1)
        n = int(sys.argv[2])
        key_file = sys.argv[3]
        key = bad_prng(n)
        ...
        if len(sys.argv) != 4:
            print("Usage: python3 bad_otp.py enc <plaintext_file> <key_file>")
            sys.exit(1)
        msg_file = sys.argv[2]
        key_file = sys.argv[3]
        ...
        if len(sys.argv) != 4:
            print("Usage: python3 bad_otp.py dec <ciphertext_file> <key_file>")
            sys.exit(1)
        ct_file = sys.argv[2]
        key_file = sys.argv[3]
