...
public abstract class Poligono {
    ...
    public boolean fechada() {
        return this.poligono.get(0).equals(
            this.poligono.get(this.poligono.size() - 1)
        );
    }

    public double perimetro() {
        double res = 0;
        for (int i = 1; i < this.poligono.size(); i++) res += this.poligono.get(
            i
        ).distancia(this.poligono.get(i - 1));
        return res;
    }
...
*/
    ...
    public boolean eConvexo() {
        if (
            this.poligono.size() < 4 // Triangulo é sempre convexo
        ) return true;
        ...
        for (int i = 0; i < n; i++) {
            double dx1 =
                poligono.get((i + 2) % n).getX() -
                poligono.get((i + 1) % n).getX();
            double dy1 =
                poligono.get((i + 2) % n).getY() -
                poligono.get((i + 1) % n).getY();
            double dx2 =
                poligono.get(i).getX() - poligono.get((i + 1) % n).getX();
            double dy2 =
                poligono.get(i).getY() - poligono.get((i + 1) % n).getY();
